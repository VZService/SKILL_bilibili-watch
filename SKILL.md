...
- **API 备用通道两个坑（2026-09-15 修复 v1.2.3）**：`fetch_subtitle_via_api()` 曾是"永远不会成功"的死代码，两处都坏了。
  - ① **aid/cid 不能用网页正则抽**：B站视频页 HTML 结构变更后，`re.search(r'"aid":(\d+)', html)` 恒为空，函数静默 `return None`。改用 `x/web-interface/view?bvid=` 取 `data.aid` / `data.cid`（需 wbi 签名），网页正则降为保底。**副作用**：靠它判断"视频有无字幕"时全是假阴性，实测 12 个热门视频全报"无字幕"，换 view 接口后同样本 32 个里 20 个有 `ai-zh`。
